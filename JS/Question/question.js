...
const multipliedArr = arr3.map(item => item * 10);
console.log(multipliedArr);

// Q3-2. filter함수와 arr3을 활용하여 [1,3,5]출력하기 (Hint:홀수와 짝수 구분은 "%2"를 활용)
const oddNumbers = arr3.filter(item => item % 2 !== 0);
...
function isLeap(year) {
  // 연도가 4로 나누어 떨어지고, 100으로 나누어 떨어지지 않거나 400으로 나누어 떨어지는 경우 윤년
...
for (let i = 1; i <= lines1; i++) {
    let space = '';
    let star = '';
    
    for (let j = 1; j <= lines1 - i; j++) {
        space += ' ';
    }
...
// 별 모양 출력
for (let i = 1; i <= lines3; i++) {
    let space = '';
    let star = '';
    // 빈 칸 추가
    for (let j = 1; j <= lines3 - i; j++) {
        space += ' ';
    }
    // 별 추가
